  ...
  return (
    <div className="flex flex-col md:flex-row bg-white p-4 m-4 md:m-10 rounded-xl border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] justify-between items-center relative z-50">
      <div className="flex justify-between items-center w-full md:w-auto">
        <Link href="/">
          <h1 className="text-3xl p-2 font-black tracking-tighter italic cursor-pointer">Voluntr<span className="text-lime-500">.</span></h1>
        </Link>
      ...
      >
        <Button variant="ghost" className="text-lg hover:bg-lime-100 hover:text-black transition-colors">
          NGOs
        </Button>
        <Button variant="ghost" className="text-lg hover:bg-lime-100 hover:text-black transition-colors">
          EVENTS
        ...
        <div className="flex flex-col md:flex-row items-center gap-4 p-2 w-full md:w-auto relative">
          {session ? (
            <div className="relative">
              ...
              >
                {session.user?.image ? (
                  ...
                  <span className="text-xl font-black">{session.user?.name?.charAt(0) || "U"}</span>
                )}
              </button>
            ...
            <Button onClick={() => router.push("/login")} className="w-full md:w-auto bg-lime-400 text-black border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:translate-y-[2px] hover:translate-x-[2px] transition-all font-bold">
              Get Started
            </Button>
          )}
        </div>
